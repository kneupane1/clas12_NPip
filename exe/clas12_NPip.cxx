...
int main(int argc, char **argv) {
  ...
  // Make your histograms object as a shared pointer that all the threads will
  // have
  ...
  // For each thread
  for (size_t i = 0; i < NUM_THREADS; i++) {
    // Get the information from the thread in this case how many events each
    // thread actually computed
  ...
  std::cout << BOLDYELLOW << events / elapsed_full.count() << " Hz" << DEF
            << std::endl;
  return 0;
}
