...
size_t run(std::shared_ptr<TChain> _chain,
           const std::shared_ptr<Histogram> &_hists, int thread_id);
size_t run_files(std::vector<std::string> inputs,
                 const std::shared_ptr<Histogram> &hists, int thread_id);
...
size_t run(std::shared_ptr<TChain> _chain,
  ...
  size_t total = 0;
  // For each event
  for (size_t current_event = 0; current_event < num_of_events;
       current_event++) {
    // Get current event
    _chain->GetEntry(current_event);
    // If we are the 0th thread print the progress of the thread every 1000
    // events
    if (thread_id == 0 && current_event % 10000 == 0)
      std::cerr << "\t" << (100 * current_event / num_of_events) << " %\r"
                << std::flush;
