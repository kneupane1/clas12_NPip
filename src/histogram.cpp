...
void Histogram::Fill_WvsQ2(const std::shared_ptr<Reaction> &_e) {
    ...
    if (_e->NPip()) {
      if (_e->W() > 1.0 && _e->W() < 2.3) {
        ...
        if (_e->MM_NPip_cut()) {
          ...
          } else if (_e->Q2() > 1.8 && _e->Q2() < 2.5) {
            W_hist_NPip_events[all_sectors][2]->Fill(_e->W());
            W_hist_NPip_events[sec][2]->Fill(_e->W());
          } else if (_e->Q2() > 2.5 && _e->Q2() < 3.0) {
            W_hist_NPip_events[all_sectors][3]->Fill(_e->W());
            W_hist_NPip_events[sec][3]->Fill(_e->W());
          } else if (_e->Q2() > 3.0 && _e->Q2() < 4.0) {
            W_hist_NPip_events[all_sectors][4]->Fill(_e->W());
            W_hist_NPip_events[sec][4]->Fill(_e->W());
          } else if (_e->Q2() > 4.0 && _e->Q2() < 5.0) {
            W_hist_NPip_events[all_sectors][5]->Fill(_e->W());
            W_hist_NPip_events[sec][5]->Fill(_e->W());
          } else if (_e->Q2() > 5.0 && _e->Q2() < 12.0) {
            W_hist_NPip_events[all_sectors][6]->Fill(_e->W());
            W_hist_NPip_events[sec][6]->Fill(_e->W());
          }
        }
      }
    }
  ...
  //       E_vs_theta_e_2nd_reso_events[all_sectors][0]->Fill(
  //           _e->theta_elec(), _e->E_elec());
  //       E_vs_theta_e_2nd_reso_events[sec][0]->Fill(_e->theta_elec(),
  //                                                  _e->E_elec());
  //     } else if (_e->W() > 1.64 && _e->W() < 1.73) {
  //       E_vs_theta_e_3rd_reso_events[all_sectors][0]->Fill(
  //           _e->theta_elec(), _e->E_elec());
  ...
  //       W_vs_q2_all_events[all_sectors][1]->Fill(_e->W(),
  //       _e->Q2());
  ...
  //       // _e->E_elec());
  //       // E_vs_theta_e_all_events[sec][1]->Fill(_e->theta_elec(),
  //       _e->E_elec());
  //       // if (_e->elastic()) {
  //       //
  ...
  //                                                   _e->E_elec());
  //     E_vs_theta_e_all_events[sec][2]->Fill(_e->theta_elec(),
  //     _e->E_elec()); if (_e->elastic()) {
  ...
  //                                                   _e->E_elec());
  //     E_vs_theta_e_all_events[sec][3]->Fill(_e->theta_elec(),
  //     _e->E_elec()); if (_e->elastic()) {
...
void Histogram::Write_WvsQ2() {
  ...
  for (int i = 0; i < NUM_SECTORS; i++) {
    ...
    for (int j = 0; j < NUM_Q2_BINS; j++) {
      ...
      // W_hist_all_events[i][j] -> Fit("gaus", "QMR+", "QMR+",
      // 0.8, 1.1); gStyle->SetOptFit(1111);
  ...
  NPip_folder->cd();
  // auto W_NPip_can = std::make_unique<TCanvas>("W NPip_can", "W
  // NPip sectors", 1920, 1080); W_NPip_can->Divide(4, 2);
  for (short i = 0; i < NUM_SECTORS; i++) {
    for (int j = 0; j < NUM_Q2_BINS; j++) {
      ...
      // auto MM2_NPip_can = std::make_unique<TCanvas>("MM2
      // NPip_can", "MM2 NPip sectors", 1920, 1080);
      // MM2_NPip_can->Divide(4, 2); for (short i = 0; i <
      // NUM_SECTORS; i++) {
