...
Cuts::Cuts(const std::shared_ptr<Branches12> &data) : _data(data) {
  _dt = std::make_shared<Delta_T>(data);
}
Cuts::Cuts(const std::shared_ptr<Branches12> &data,
           const std::shared_ptr<Delta_T> &dt)
    : _data(data), _dt(dt) {}
Cuts::~Cuts() {}
...
bool Cuts::ElectronCuts() {
  ...
  _elec &= (_data->vz(0) > -(2.78 + 3 * 2.16) &&
            _data->vz(0) < (-2.78 + 3 * 2.16)); // 3 sigma cut
  // Use the chi2pid instead of straight line cuts on SF
  //_elec &= (abs(_data->chi2pid(0)) < 3);

  _elec &=
      (_data->ec_tot_energy(0) / _data->p(0) <
       (0.30676 - 0.00111 * _data->p(0) - 0.00031 * _data->p(0) * _data->p(0)));
  _elec &=
      (_data->ec_tot_energy(0) / _data->p(0) >
       (0.15546 + 0.01714 * _data->p(0) - 0.00151 * _data->p(0) * _data->p(0)));
...
bool Cuts::FiducialCuts() {
  ...
  float x_PCAL_rot =
      _data->ec_pcal_y(0) * sin_dc_sec + _data->ec_pcal_x(0) * cos_dc_sec;
  float y_PCAL_rot =
      _data->ec_pcal_y(0) * cos_dc_sec - _data->ec_pcal_x(0) * sin_dc_sec;

  float left_PCAL = (HEIGHT_PCAL - SLOPE * y_PCAL_rot);
  float right_PCAL = (HEIGHT_PCAL + SLOPE * y_PCAL_rot);
  float radius2_PCAL =
      X_SQUARE_PCAL -
      (y_PCAL_rot * y_PCAL_rot); // circle radius r^2 = x^2 + y^2
  ...
  float x1_rot =
      _data->dc_r1_y(0) * sin_dc_sec + _data->dc_r1_x(0) * cos_dc_sec;
  float y1_rot =
      _data->dc_r1_y(0) * cos_dc_sec - _data->dc_r1_x(0) * sin_dc_sec;
  ...
  float x2_rot =
      _data->dc_r2_y(0) * sin_dc_sec + _data->dc_r2_x(0) * cos_dc_sec;
  float y2_rot =
      _data->dc_r2_y(0) * cos_dc_sec - _data->dc_r2_x(0) * sin_dc_sec;
  ...
  float x3_rot =
      _data->dc_r3_y(0) * sin_dc_sec + _data->dc_r3_x(0) * cos_dc_sec;
  float y3_rot =
      _data->dc_r3_y(0) * cos_dc_sec - _data->dc_r3_x(0) * sin_dc_sec;
...
bool Cuts::IsPositive(int i) {
  ...
  // if (!std::isnan(_dt->dt_ctof_P(i))) _proton &= (abs(_dt->dt_ctof_P(i)) <
  // 0.2);
  return _pos;
}
...
bool Cuts::IsProton(int i) {
  ...
  _proton &= (_data->p(i) > 1.0);
  if (!std::isnan(_dt->dt_P(i)))
    _proton &= (abs(_dt->dt_P(i)) < 0.5);
  if (!std::isnan(_dt->dt_ctof_P(i)))
    _proton &= (abs(_dt->dt_ctof_P(i)) < 0.2);
  return _proton;
}
